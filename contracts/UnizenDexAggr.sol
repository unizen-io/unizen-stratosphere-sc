...
contract UnizenDexAggr is IUnizenDexAggr, Controller, EthReceiver, ReentrancyGuardUpgradeable, IStargateReceiver {
    ...
    function sgReceive(
        ...
        if (dstToken == address(0)) {
            ...
            if (_nonce > 0) {
                if (_nonce > actualQuote) {
                    _nonce = _nonce - _takePSFee(dstToken, (_nonce - actualQuote), userPSFee, uuid, 0);
                }
    ...
    function swapExactOut(
        ...
        if (isETHTrade) {
            // trade from ETH
            contractStatus.balanceSrcBefore = address(this).balance; // eth balance of contract
        } else {
            contractStatus.balanceSrcBefore = srcToken.balanceOf(address(this));
        }
        ...
        if (isETHTrade) {
            contractStatus.balanceSrcAfter = address(this).balance; // eth balance of contract
        } else {
            contractStatus.balanceSrcAfter = srcToken.balanceOf(address(this));
        }
        uint256 diff = contractStatus.balanceSrcAfter + amountTakenIn - contractStatus.balanceSrcBefore; // remaining funds from to be sent back
        uint256 swappedAmount = contractStatus.balanceSrcBefore - contractStatus.balanceSrcAfter; // total src amount swap

        if (diff > 0) {
            if (info.srcToken != address(0)) {
                if (swappedAmount < info.actualQuote) {
                    diff =
                        diff -
                        _takePSFee(
                            info.srcToken,
                            (info.actualQuote - swappedAmount),
                            info.userPSFee,
                            info.uuid,
                            info.feePercent
                        ); // remaining funds from slippage to be sent back
                }
                srcToken.safeTransfer(msg.sender, diff);
            } else {
                if (swappedAmount < info.actualQuote) {
                    diff =
                        diff -
        ...
        emit Swapped(
            swappedAmount, //actual swapped amount,
    ...
    function swap(SwapExactInInfo memory info, SwapCall[] memory calls) external payable whenNotPaused nonReentrant {
        ...
        if (totalDstAmount > info.actualQuote) {
            totalDstAmount =
                totalDstAmount -
                _takePSFee(
                    info.dstToken,
                    (totalDstAmount - info.actualQuote),
                    info.userPSFee,
                    info.uuid,
                    info.feePercent
                );
        }
        require(totalDstAmount >= info.amountOutMin, "Return amount is not enough");

        if (info.dstToken != address(0)) {
            dstToken.safeTransfer(info.receiver, totalDstAmount);
        } else {
    ...
    function setPSFee(uint256 _psFee, uint256 _psShare) external onlyOwner {
        psFee = _psFee;
        psShare = _psShare;
    }
    ...
    function _takePSFee(
        ...
        uint256 psAmount = (amountSlippage * psFee) / 10000; // amount unizen + kols
        if (bytes(uuid).length == 0) {
            if (userPSFee > 0) {
                psAmount = psAmount - (psAmount * userPSFee) / 10000; // share back to user amount based on userPSFee (level)
            }
            // no uuid
        ...
        if (uuidType[uuid] == 2) {
            // KOLs
            integratorPSEarned[integratorAddrs[uuid]][token] += (psAmount * psShare) / 10000;
            amountSlippage = psAmount;
            psAmount = psAmount - (psAmount * psShare) / 10000;
            if (userPSFee > 0) {
                amountSlippage = amountSlippage - (psAmount * userPSFee) / 10000;
                psAmount = psAmount - (psAmount * userPSFee) / 10000; // share back to user amount based on userPSFee (level)
            }
            _psEarned[token] = _psEarned[token] + psAmount;
            return amountSlippage;
        } else if (uuidType[uuid] == 1) {
            // integrator
            if (feePercent <= limitShare) {
                // if they take fee less than or equal limitShare, we share them PS
                integratorPSEarned[integratorAddrs[uuid]][token] += (amountSlippage - psAmount);
                if (userPSFee > 0) {
                    amountSlippage = amountSlippage - (psAmount * userPSFee) / 10000;
                    psAmount = psAmount - (psAmount * userPSFee) / 10000; // share back to user amount based on userPSFee (level)
                }
